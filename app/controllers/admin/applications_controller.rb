module Admin
  class ApplicationsController < Admin::ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @application.save
            format.html { redirect_to(admin_applications_path, notice: 'Application 创建成功。') }
            format.json
          else
    ...
    def update
      ...
      respond_to do |format|
        if @application.update_attributes(params[:application].permit!)
            format.html { redirect_to(admin_applications_path, notice: 'Application 更新成功。') }
            format.json
          else
    ...
    def destroy
      ...
      respond_to do |format|
          format.html { redirect_to(admin_applications_path, notice: '删除成功。') }
        format.json
      end
    end
  end
