module Admin
  class SitesController < ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @site.save
          format.html { redirect_to(admin_sites_path, notice: 'Site 创建成功。') }
          format.json
        else
    ...
    def update
      ...
      respond_to do |format|
        if @site.update_attributes(params[:site].permit!)
          format.html { redirect_to(admin_sites_path, notice: 'Site 更新成功。') }
          format.json
        else
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to(admin_sites_path, notice: "#{@site.name} 删除成功。") }
        format.json
      end
    end
