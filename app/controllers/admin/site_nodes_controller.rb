module Admin
  class SiteNodesController < ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @site_node.save
          format.html { redirect_to(admin_site_nodes_path, notice: 'Site node 创建成功。') }
          format.json
        else
    ...
    def update
      ...
      respond_to do |format|
        if @site_node.update_attributes(params[:site_node].permit!)
          format.html { redirect_to(admin_site_nodes_path, notice: 'Site node 更新成功。') }
          format.json
        else
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to(admin_site_nodes_path, notice: '删除成功。') }
        format.json
      end
    end
  end
